  ...
  try {
    ...
    for ( var i = 0; i < ocs.length; i++ ) {
      ...
      oc.buyItems.forEach( function (item){
        if (item.resellerId && item.checked) {
          ...
          if (resellerMerchantId) {
            ...
            if (resellerPrice) {
              ...
              else if (resellerPrice.fixedRate) {
        ...
        for(var cartKey in ruleResult.ocPayPrices){
          for(var i=0; i<ocs.length; i++){
            ...
            if(oc.cartKey == cartKey){
              ...
              var ratio = ocPayPrice / totalPayPrice;
              var sharedCards = ruleUsedCards.map(function(card){
                return {cardId:card.cardId,ratio:ratio,amount:Number(cart.amount)*ratio}
              });
              oc.crossMerchantUsedCards = oc.crossMerchantUsedCards.concat(sharedCards);
              var cardTotalAmount = 0;
    ...
    ocs.forEach( function (oc){
      ...
      //真正保存订单
        var orderId = "" + NormalBuyFlowService.addRealOrder(order, order.orderType, buyerId);
