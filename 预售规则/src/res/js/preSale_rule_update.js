function PreSaleRuleUpdatePage(configs) {
    var self = this;
    self.count = ko.observable(0);
    self.id = ko.observable(configs.id || "");
    self.name = ko.observable(configs.name || "");
    self.rate = ko.observable(configs.rate || 1);
    self.displayAmount = ko.observable(configs.displayAmount || "");
    self.deposit = ko.observable(configs.deposit || "");
    self.balance = ko.observable(configs.balance || "");
    self.totalPrice = ko.observable(configs.totalPrice || "");
    self.type = ko.observable(configs.type || "");
    self.depositBeginTime = ko.observable();
    self.depositEndTime = ko.observable();
    self.beginTime = ko.observable(configs.beginTime || "");
    self.endTime = ko.observable(configs.endTime || "");
    self.stockingTime = ko.observable(configs.stockingTime || "");
    self.createUserId = ko.observable(configs.createUserId || "");
    self.desc = ko.observable(configs.desc || "");
    self.products = ko.observableArray(configs.products || []);
    self.scope = ko.observableArray(configs.scope || []);
    self.merchantId = ko.observable(configs.merchantId || "");
    self.createTime = ko.observable(configs.createTime || "");
    self.approveState = ko.observable(configs.approveState || "0");
    ...
    self.save = function () {
        ...
        if (!postData.beginTime || postData.beginTime.trim() == "") {
            bootbox.alert("尾款支付开始时间不能为空。");
            return;
        }
        if(!reg.test(postData.beginTime.trim())){
            bootbox.alert("请设置完整的尾款支付开始时间。");
            return;
        }

        if (!postData.endTime || postData.endTime.trim() == "") {
            bootbox.alert("尾款支付结束时间不能为空。");
            return;
        }
        if(!reg.test(postData.endTime.trim())){
        ...
        if (type == "2") {
            ...
            if (type == "1") {
                if (postData.balance != 0 && (!postData.balance || postData.balance == "")) {
                    bootbox.alert("尾款不能为空。");
                    return;
                }
        ...
        $.post("handler/preSale_rule_update_handler.jsx", {m: m, appStr: JSON.stringify(postData)}, function (ret) {
            if (ret.state == 'ok') {
...
var preSaleRuleUpdatePage = null;
$(document).ready(function () {
    var ruleId = $("#ruleId").val();
    $.post("handler/get_preSale_rule.jsx", {m: m, id: ruleId}, function (ret) {
        var pres = ret.data;
        pres.products = pres.items;
        preSaleRuleUpdatePage = new PreSaleRuleUpdatePage(pres);
        ko.applyBindings(preSaleRuleUpdatePage, document.getElementById("preSaleRuleUpdatePage"));
        ...
        if (type == "2") {
            ...
            $("#sections").show();
            $("#btnDiv").show();
            var scopes = pres.scope;
            for (var i = 0; i < scopes.length; i++) {
                var item = scopes[i];
                var subData = {};
                subData.begin = item.begin;
                subData.end = item.end;
                subData.scopeBalance = item.scopeBalance;
                subData.scopeTotalPrice = item.scopeTotalPrice;
                addContent(subData);
            }
        }else if(type == "3"){
            $("#totalPriceDiv").show();
        ...
        if(pres.approveState != "0"){
             //如果已审核，那么当前时间之前的日期都不能选择
             var date = new Date();
