...
//#import inventory.js
(function () {
    if (typeof isRun == "undefined" || typeof titles == "undefined" || typeof m == "undefined") {
        return;//do nothing
    }
    ...
    function loadPoduct(keyword, beginCreateTime, endCreateTime) {
        ...
        for (var i = 0; i < productIds.length; i++) {
            ...
            if (product.noversion.certifyState) {
                ...
                else if (product.noversion.certifyState == "2") {
    ...
    function createExcel(titles, fileName, m, keyword, beginCreateTime, endCreateTime) {
        titles = JSON.parse(titles);
        var product = loadPoduct(keyword, beginCreateTime, endCreateTime);
        titles = setTitles(titles);
        var export_file_type = "outPutProductListByTitle";
        var s = Excel.createExcelList(m, fileName, export_file_type, titles, product);
    }
    if (isRun == "true") {
        createExcel(titles, fileName, m, keyword, beginCreateTime, endCreateTime);
    }
    else {
        return;
    }
