...
(function () {
  ...
  //  var eventId = $.params["eventId"];
   // var activityGrade = $.params["activityGrade"];
    if (!currentPage) {
        currentPage = 1;
    }
    ...
    if (id!='undefined'&&id!=null) {
        var listId =   LotteryLogService.getAwardListByActiveId(id);
        record=LotteryLogService.getListByName(listId,0,-1);
        for (var i = 0; i < record.length; i++) { //时间处理
            var jRecord = record[i];
            if (!jRecord) {
                continue;
            }
        ...
        if (isSearch) {
            ...
            totalRecords = LotteryLogService.getAllListSize();
            listData = LotteryLogService.getAllList(start, pageLimit);
        }
    ...
    var template = $.getProgram(appMd5, "pages/load_record_lotteryLog.jsxp");
