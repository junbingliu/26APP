...
//#import $lotteryEventManage:services/LotteryEventManageQuery.jsx
...
(function () {
    ...
    if (isSearch) {
        ...
        searchArgs.queryArgs = LotteryEventManageQuery.getQueryArgs(searchParams);
        ...
        for (var i = 0; i < ids.size(); i++) {
            var objId = ids.get(i);
            var record = LotteryEventManageService.get(objId);
            if (record) {
                listData.push(record);
            }
        }
    } else {
        totalRecords = LotteryEventManageService.getAllListSize();
        listData = LotteryEventManageService.getAllList(start, pageLimit);
    }
