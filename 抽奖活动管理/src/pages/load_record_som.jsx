...
//#import $lotteryEventManage:services/PrizeSettingSomQuery.jsx
...
(function () {
    ...
    var recordList = [];
    if (id!='undefined' && id!='' && keyword ==null){
        var listId =   PrizeSettingSomService.getAwardListByActiveId(id);
        ...
        if (isSearch) {
            ...
            searchArgs.queryArgs = PrizeSettingSomQuery.getQueryArgs(searchParams);
            ...
            for (var i = 0; i < ids.size(); i++) {
                var objId = ids.get(i);
                var record = PrizeSettingSomService.get(objId);
                if (record) {
                    listData.push(record);
                }
            }
        } else {
            totalRecords = PrizeSettingSomService.getAllListSize();
            listData = PrizeSettingSomService.getAllList(start, pageLimit);
        }
    ...
    var template = $.getProgram(appMd5, "pages/load_record_som.jsxp");
