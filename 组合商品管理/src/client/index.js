...
self.pager.pageSize(50);
self.pager.currentPage(0);
self.certified=ko.observable("F");
self.published=ko.observable("F");
self.mID=ko.observable(m);//后台登录账户id
self.search = function(start,limit){
$.post("server/searchMyProducts.jsx",{keyword:self.keyword(),m:m,start:start,limit:limit,certified:self.certified(),published:self.published()},function(ret){
self.pager.total(ret.total);
self.pager.setStart(start);
self.combiProducts($.map(ret.combiProducts,function(p){
if(p.createTime){
...
var findProductsPage = null;
$(function(){
findProductsPage = new FindProductsPage();
ko.applyBindings(findProductsPage,document.getElementById("findProductsPage"));
});
...
this.get("#/listProducts",function(context){
showPage("listProductsPage");
listProductsPage.doSearch();
$("#mainMenu li").removeClass("active");
$("#menuItemListProducts").addClass("active");
});
this.get("#/listProductsPage/help",function(context){
showPage("listProductsHelpPage");
});
this.get("#/about",function(context){
showPage("aboutPage");
$("#mainMenu li").removeClass("active");
$("#menuItemAbout").addClass("active");
});
this.get("#/addCombiProduct",function(context){
productForm.clear();
showPage("productFormPage");
$("#mainMenu li").removeClass("active");
$("#menuItemAddCombiProduct").addClass("active");
});
this.get("#/aboutFangXing",function(context){
aboutFangXing.getAbouts();
showPage("aboutFangXingPage");
$("#mainMenu li").removeClass("active");
$("#menuItemFangXing").addClass("active");
});
this.get("#/findCombiProduct",function(context){
showPage("findProductsPage");
findProductsPage.doSearch();
$("#mainMenu li").removeClass("active");
$("#menuItemFindCombiProduct").addClass("active");
});
this.get("#/editCombiProduct/:productId",function(context){
var productId =  this.params['productId'];
console.log(productId);
productForm.setCombiProductId(productId);
showPage("productFormPage");
$("#mainMenu li").removeClass("active");
$("#menuItemAddCombiProduct").addClass("active");
});
});
sammyApp.run("#/listProducts");
}

define(["/koComponents/modules/priceEditor/priceEditor.jsx"],function(PriceEditor){
window.PriceEditor = PriceEditor;
main();
});
