...
//#import $inventoryManage:services/InventoryManageQuery.jsx
...
(function () {
    ...
    if (isSearch) {
        ...
        var queryArgs = InventoryManageQuery.getQueryArgs(searchParams);
        //排序条件，按创建时间倒序排
        ...
        for (var i = 0; i < ids.size(); i++) {
            var objId = ids.get(i);
            var record = InventoryManageService.getById(objId);
            if (record) {
                resultList.push(record);
            }
        }
    } else {
        totalRecords = InventoryManageService.listCount();//获取列表总数
        //获得所有记录
        resultList = InventoryManageService.list(start, pageLimit);
    }
    ...
    if (resultList && resultList.length > 0) {
        ...
        for (var i = 0; i < resultList.length; i++) {
            ...
            var jMerchant = MerchantService.getMerchant(jPres.merchantId);
            if (jMerchant) {
                jPres.merchantName = jMerchant.name_cn;
            }
            var jUser = UserService.getUser(jPres.userId);
            if (jUser) {
                jPres.loginId = jUser.loginId;
            } else {
                jPres.loginId = "系统";
            }
            if (jPres.shipNode == "zeroSellableCount") {
                jPres.shipNode = "零负可卖数";
            } else {
                jPres.shipNode = "实际库存";
            }
            var jProduct = ProductService.getProduct(jPres.productId);
            if (jProduct) {
                jPres.productName = jProduct.name;
            }
            if (jPres.createTime)
                jPres.createTime = DateUtil.getLongDate(jPres.createTime);
        }
    } else {
        resultList = [];
    }
