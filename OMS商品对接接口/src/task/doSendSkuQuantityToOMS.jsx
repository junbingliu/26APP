...
(function () {
    ...
    var productList = OmsEsbProductService.getQtyExchangeList(merchantId,0,-1);//获取对接队列
    //即使当天没有库存变化，也要将第二天的task加到队列里
    if (productList.length == 0) {
        add2Task(merchantId);
        return;
    }

    //$.log("\n.............................doSendSkuQuantityToOMS.jsx....8888888888888888888888...productList.length=" + productList.length);
    ...
    for (var k = 0; k < productList.length; k++) {
        ...
        //将商品从对接队列删除
        OmsEsbProductService.deleteFormExchangeList(merchantId,productId);
        ...
        if (skus.length > 1) {
            for (var i = 0; i < skus.length; i++) {
                ...
                //skuList = getZeroSellCount(skuList, jSku, productId, defaultShipNode, formatCurTime);
            }
        } else {
            jSku = skus[0];
            skuList = getSmallSku(skuList, jSku, formatCurTime, merchantId);
...
function add2Task(merchantId) {
    ...
    var when = getNextExecTime();//每小时执行一次
    var postData = {
        merchantId: merchantId
    };
...
function getNextExecTime() {
    ...
    var shortDate = DateUtil.getShortDate(nowTime);//当天日期
    var shortTime = DateUtil.getLongTime(shortDate);
    var executeTime = parseInt(shortTime) + 29 * 60 * 60 * 1000;//每天凌晨5点执行
    return executeTime;
}
