...
var CartUtil = (function () {
    ...
    var f = {
        addToCart: function (productId, skuId, amount, uid, spec) {
            ...
            try {
                ...
                if (integralRule) {
                    ...
                    ret = f.addIntegralToCart(productId, skuId, amount, uid, spec);
                    ret.cartType = cartType;
                    return ret;
                }
                if (productId.indexOf("combiProduct") > -1) {
                    ...
                    for (var idxItems = 0; idxItems < combiProduct.productItems.length; idxItems++) {
                        ...
                        var subItem = {
                            ...
                            deliveryWay: product.deliveryWay || "0"
                        };
                ...
                if (cart.buyItems) {
                    cart.buyItems.forEach(function (item) {
                        ...
                        /*if (item.objType === "integral") {
                            var cxt = "{attrs:{},factories:[{factory:INPF},{factory:RPF,moneyTypeId:'moneytype_INTEGRAL',direction:'1'},{factory:MF}]}";//市场价和积分价
                            var newProductPrices = ProductService.getPriceValueList(item.productId, userId, oc.getMerchantId() + "", item.amount, cxt, "normalPricePolicy");
                            var price = newProductPrices[0];
                            item.unitPrice = Number(price.unitPrice/100).toFixed(2);
                            item.totalPrice = Number(price.totalPrice/100).toFixed(2);
                            item.integralPrice = Number(price.integralPrice/100).toFixed(2);
                            item.totalIntegralPrice = Number(price.totalIntegralPrice/100).toFixed(2);
                        }*/
                        //单笔限购
                        var prefix = "limitBuy";
            ...
            for (var i = 0; i < carts.length; i++) {
                ...
                for (var k = 0; k < buyItems.length; k++) {
                    ...
                    var newBuyItem = {
                        ...
                        integralPrice: buyItem.integralPrice,
                        totalIntegralPrice: buyItem.totalIntegralPrice,
                        objType: buyItem.objType
                    };
                    var diffPrice = 0;
                ...
                newCart = {
                    ...
                    totalIntegralPrice: cart.totalIntegralPrice,
         ...
         * @returns {*}
         */
        getOleVirtualOrderDeliveryRuleId: function () {
            return SysArgumentService.getSysArgumentStringValue("head_merchant", "col_sysargument", "ole_virtualOrderDeliveryRuleId");
        },
        ...
        addIntegralToCart: function (productId, skuId, amount, uid, spec) {
            var ret = ErrorCode.S0A00000;
            if (!productId) {
                ret = ErrorCode.E1M000000;
                return ret;
            ...
            if (Number(sellCount) < (Number(amount) + Number(countInCart))) {
                ret = ErrorCode.cart.E1M050094;
                return ret;
            }
            ...
            var bigCart = CartService.getBigCart();
            CartUtil.unCheckCartByType(bigCart, "preSale");//将预售购物车不选中
            CartService.updateBigCart(bigCart);//保存购物车

            //TODO:检查购物车中的数量，看看加起来是否有库存
            var jShoppingCart = Api.IsoneOrderEngine.shoppingCart.getShoppingCart(request, response, true);

            var integralPrice = parseFloat(integralRule.jifen);//积分价
            var cashPrice = parseFloat(integralRule.money).toFixed(2);
            var productPrice = cashPrice + "+" + integralPrice;
            ...
            jItem.put("cardBatchId", product["cardBatchId"]);
            jItem.put("isVirtual", product["isVirtual"]);
            jItem.put("productPrice", productPrice);
            try {
                Api.ShoppingCartUtil.addItem(jShoppingCart, jItem);
            } catch (e) {
                // $.log("\n................................e="+e);
