...
//#import DateUtil.js
//#import PreSale.js
//#import $oleH5Api:services/OrderListQuery.jsx
...
(function () {
    ...
    // try {
        ...
        if ("getOrderDetails" === serviceCode) {
            ...
            searchParams.merchant_otherColumn = ColumnService.getColumnIdPath("c_ole_store_100", "rootcolumn", "/");//搜索所有OLE门店商家的订单
...
function getOrderDetails(jOrder) {
    ...
    if (refundState) {
        refundStateInfo = {state: refundStateInfo.state, name: getRefundStateName(refundState.state)}
    }
    ...
    for (var key in items) {
        ...
        tmpItem.realSkuId = jItem.realSkuId;
        ...
        if (jItem.priceInfo.fTotalPrice && jItem.priceInfo.fTotalPrice != "" && jItem.priceInfo.fTotalPrice != undefined) {
            tmpItem.totalPrice = jItem.priceInfo.fTotalPrice || "";  //订单中各商品的总价格
        } else {
            tmpItem.totalPrice = ""
        }
        if (jItem.priceInfo.fUnitPrice && jItem.priceInfo.fUnitPrice != "" && jItem.priceInfo.fUnitPrice != undefined) {
            tmpItem.unitPrice = jItem.priceInfo.fUnitPrice || "";  //订单中各商品的单价
            // tmpItem.fIntegralPrice = jItem.priceInfo.fGiveProductIntegral || "";  //订单中各商品的积分
            tmpItem.fIntegralPrice = jItem.priceInfo.fIntegralRMBPrice || "";  //订单中各商品的积分
        } else {
            tmpItem.unitPrice = "";
            mpItem.fIntegralPrice = "";
        }
        if (jItem.priceInfo.fSupplyPrice && jItem.priceInfo.fSupplyPrice != "" && jItem.priceInfo.fSupplyPrice != undefined) {
            tmpItem.supplyPrice = jItem.priceInfo.fSupplyPrice || "";  //供货价
        } else {
            tmpItem.supplyPrice = ""
        }
        if (jItem.priceInfo.fMarketPrice && jItem.priceInfo.fMarketPrice != "" && jItem.priceInfo.fMarketPrice != undefined) {
            tmpItem.marketPrice = jItem.priceInfo.fMarketPrice || "";  //市场价
        } else {
            tmpItem.marketPrice = ""
        }
        tmpItem.moneyTypeName = jItem.moneyTypeName;
    ...
    jSmallOrder.payStateInfo = payStateInfo;           //订单支付状态     p200：待支付【付款】【取消订单】    p201: 已支付
    ...
    jSmallOrder.afterServiceStateInfo = afterServiceStateInfo;  //订单退换货状态
    ...
    if (afterSatateInfo.state) {
        jSmallOrder.afterState = afterSatateInfo.state;  //售后状态  退款中 已完成
    }
    if (afterSatateInfo.refundAliasCode) {
        jSmallOrder.refundAliasCode = afterSatateInfo.refundAliasCode;//退换货的单号
    }
    return jSmallOrder;
}
...
function getAfterState(orderId) {
    ...
    if (refunded_cancel && refunded_cancel.length > 0) {//取消退款单
        ...
        for (var i = 0; i < refunded_cancel.length; i++) {
            ...
            refundAliasCode = getrefundAliasCode(record);//退货退款单号
    ...
    if (refunded_reason && refunded_reason.length > 0) {//其他原因退款单
        ...
        for (var i = 0; i < refunded_reason.length; i++) {
            ...
            refundAliasCode = getrefundAliasCode(record);//退货退款外部单号
            var jRefundDoc = AfterSaleService.getRefundOrder(record.id);//退款单
            //已取消的其他原因退款单不用理
    ...
    if (after_return && after_return.length > 0) {//退货单
        ...
        for (var i = 0; i < after_return.length; i++) {
            ...
            // $.log("=====================>"+jRefundDoc.states.approveState.state);
            //已取消的退货单不用理
            if (jRefundDoc.states.approveState && jRefundDoc.states.approveState.state == "state_2") {
                continue;
            }
            ...
            // $.log("=================================================================================");
            // $.log("jRefundDoc.states.approveState.state=============>"+jRefundDoc.states.approveState.state);
            // $.log("jRefundDoc.states.warehousingState.state=========>"+jRefundDoc.states.warehousingState.state);
            // if (jRefundDoc.states.approveState && jRefundDoc.states.approveState.state == "state_1"&&jRefundDoc.states.warehousingState&&jRefundDoc.states.warehousingState.state==="Warehousing_N") {
            //     //审核通过且为入库，为可填写客户退换货物流信息状态
            //     state="审核通过";
            // }
    ...
    if (after_barter && after_barter.length > 0) {//退货单
        $.log("after_barter==========>" + JSON.stringify(after_barter));
        state = "售后处理中";
        var isAllRefund = true;//是否所有退款单都退款完成
        var hasEffectiveDoc = false;//是否有效的退货单
        for (var i = 0; i < after_barter.length; i++) {
            var record = after_barter[i];
            refundAliasCode = getrefundAliasCode(record);//退货退款单号
            var jRefundDoc = AfterSaleService.getOrder(record.id);//换货单
            // $.log("jRefundDoc========换货单==待测试=============>\n"+JSON.stringify(jRefundDoc)+"\n=======================================================");
            // $.log("=====================>"+jRefundDoc.states.approveState.state);
            //已取消的退货单不用理
            if (jRefundDoc.states.approveState && jRefundDoc.states.approveState.state == "state_2") {
                continue;
        ...
        //如果没有有效的换货单，那就说明所有的换货单都是已取消的，不用计算状态
        if (!hasEffectiveDoc) {
            state = "";
        }
    }
    return {state: state, refundAliasCode: refundAliasCode};
}

function getrefundAliasCode(refundObj) {
    if (!refundObj) {
        return;
    }
    if (!refundObj.aliasCode) {
        return;
    }
    return refundObj.aliasCode;
}
