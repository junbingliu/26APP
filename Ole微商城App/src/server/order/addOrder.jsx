...
(function () {
    ...
    var storeMerchantId = $.params.storeMerchantId || "";//提货门店
    var selfDeliveryMobile = $.params.selfDeliveryMobile || "";//自提联系手机号码
    var selfDeliveryDate = $.params.selfDeliveryDate || "";//提货时间
    var deliveryDate = $.params.deliveryDate || "";//发货时间
    ...
    try {
        ...
        beginTime = new Date().getTime();
        //如果不是赠品订单，并且没有配送地址
        if (!defaultAddress && !isGift && orderType != "virtual") {
        ...
        for (var i = 0; i < ocs.length; i++) {
            ...
            //如果没有选择配送规则
            if (!oc.selectedDeliveryRuleId && orderType != "virtual") {
        ...
        ocs.forEach(function (oc) {
            ...
            if (orderType == "virtual") {
                //虚拟订单设置为无需配送地址
                oc.notDelivery = true;
                oc.orderType = "common";////虚拟订单只在购物车中区分，虚拟商品下单后以普通订单存在
                oc.selectedDeliveryRuleId = CartUtil.getOleVirtualOrderDeliveryRuleId();
            }
            oc.isGift = isGift;
            ...
            if(storeMerchantId){
                order.isSelfDeliveryOrder = "Y";//是否自提订单标记
                order.storeMerchantId = storeMerchantId;
                order.selfDeliveryMobile = selfDeliveryMobile;
                order.selfDeliveryDate = selfDeliveryDate;
            }
            ...
            if (orderType == "virtual") {
                //虚拟订单设置为无需配送地址
                orderType = "common";
                order.orderType = orderType;////虚拟订单只在购物车中区分，虚拟商品下单后以普通订单存在
                order.isVirtualOrder = "Y";//是否虚拟订单
            }
