...
;(function () {
    ...
    userId = SessionService.getSessionValue("orderUserId", request) || user && user.id;
    ...
    var webUrl = SysArgumentService.getSysArgumentStringValue("head_merchant", "col_sysargument", "ole_wechatUrl");
    var oleWechatUrl = webUrl + "#/OrderListPage";
    if (lst.length == 0) {
        // out.print("所有订单都已经支付");
        response.sendRedirect(oleWechatUrl);
        return;
    }
    var payResult = lst[0];
    var payment = null;
    ...
    if (mode == "app") {
        ...
        if (realPayRec.payInterfaceId == 'payi_129') {
            ...
            //去支付
            var templateSource = $.getProgram(appMd5, "server/pay/payHandler.html");
