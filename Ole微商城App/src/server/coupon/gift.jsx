...
(function () {
    ...
    var presentStatus = jCard.presentStatus || "0";//转赠状态，0：未转赠，1：转赠中，未被领，2：已领取,3:取消转赠
    if (presentStatus == "1") {
        H5CommonUtil.setErrorResult(ErrorCode.voucher.E1B13010);
        return;
    }
    if (presentStatus == "2") {
    ...
    CardService.updateCashCard(jCard.id, "presentStatus", "1");//将券修改为转赠中
    ...
    var iv = H5CommonUtil.getRanString(8);//8位随机数
    var code = DESEncryptUtil.encSign(appKey, iv, JSON.stringify(data));

    H5CommonUtil.setSuccessResult({data: code + "", iv: iv});
})();