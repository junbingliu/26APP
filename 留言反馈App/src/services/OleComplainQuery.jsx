//#import Util.js

var OleComplainQuery = (function () {
    var f = {
        getQuery: function getQuery(searches) {
            if (typeof searches === "undefined") {
                return [];
            }
            var searchQuery = [];
            //索引对象类型
            searchQuery.push({n: 'ot', v: "oleComplainObject", type: 'term'});

            //关键字
            if (searches.keyword && searches.keyword !== "") {
                searchQuery.push({n: 'keyword_text', v: searches.keyword, type: 'text', op: "and"});
            }
            if (searches.handleState && searches.handleState !== "") {
                searchQuery.push({n: 'handleState', v: searches.handleState, type: 'text', op: "and"});
            }
            if (searches.relevanceMid && searches.relevanceMid !== "") {
                searchQuery.push({n: 'relevanceMid', v: searches.relevanceMid, type: 'text', op: "and"});
            }
            if (searches.createUserId && searches.createUserId !== "") {
                searchQuery.push({n: 'createUserId', v: searches.createUserId, type: 'text', op: "and"});
            }
            return searchQuery;
        },
        getQueryArgs: function (searchParams) {
            var qValues = f.getQuery(searchParams);
            var queryArgs = {
                mode: 'adv',
                q: qValues
            };

            return JSON.stringify(queryArgs);
        }


    };
    return f;

})();