...
$(function (){
    ...
    var formValidate = bindCardForm.validate({
        ...
        submitHandler: function (form) {
            $(form).ajaxSubmit({
                type:"post",
                url:"/" + rappId + "/handler/vip_card_bind_handler.jsx",
                dataType:"json",
    ...
    $("#getValidationCode").on("click",function(){
        ...
        var mobile = $("input[name='mobile']",bindCardForm);
        if(mobile.val() == ""){
        ...
        $.get("/" + rappId + "/handler/vip_card_validate_msg_handler.jsx",{mobile:mobile.val(),t:Math.random(),token:$("input[name='token']",bindCardForm).val()},function(resp){
            if(resp.state){
